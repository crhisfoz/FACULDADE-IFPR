	...
	public function deletar(Request $request, Response $response, array $args): Response
	{
		...
		if ($veiculo) {
			try {
				...
				// Resposta de sucesso
				$mensagem = ["mensagem" => "Veículo deletado com sucesso"];
				...
				// Em caso de erro ao deletar o veículo, este bloco será executado
				$jsonErro = MensagemErro::getJSONErro("Erro ao deletar o veículo!", $ex->getMessage());
				$response->getBody()->write($jsonErro);
				return $response
					->withHeader('Content-Type', 'application/json')
					->withStatus(500); // INTERNAL_SERVER_ERROR
